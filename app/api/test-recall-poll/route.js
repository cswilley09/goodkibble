/*
  ...
  Copy of /api/cron/poll-recalls with auth removed.
...
async function scrapeFDAOutbreaks() {
  ...
  try {
    ...
    console.log(`[test-poll] FDA Outbreaks: found ${results.length} advisories`);
    return results;
  } catch (err) {
    console.error('[test-poll] FDA Outbreaks error:', err.message);
    return [];
  }
}
...
async function scrapeFDARSS() {
  ...
  try {
    ...
    console.log(`[test-poll] FDA RSS: found ${results.length} pet-related recalls`);
    return results;
  } catch (err) {
    console.error('[test-poll] FDA RSS error:', err.message);
    return [];
  }
}
...
export async function GET(request) {
  ...
  try {
    ...
    console.log(`[test-poll] Total from both sources: ${allResults.length}`);
    ...
    if (rows.length > 0) {
      ...
      console.log(`[test-poll] Inserted ${insertedCount} recalls`);
      ...
      if (inserted && inserted.length > 0) {
        for (const recall of inserted) {
          ...
          if (affectedUsers && affectedUsers.length > 0) {
            ...
            else console.log(`[test-poll] Queued ${alertRows.length} alerts for ${recall.recall_number}`);
          }
        }
      }
    }
